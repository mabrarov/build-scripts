...
foreach ($address_model in ${address_models}) {
  ...
  switch (${env:OPENSSL_ADDRESS_MODEL}) {
    "32" {
      $env:MSVC_CMD_BOOTSTRAP_OPTIONS = "x86"
  ...
  foreach ($openssl_linkage in ${openssl_linkages}) {
    ...
    foreach ($openssl_build_type in ${openssl_build_types}) {
      ...
      $env:OPENSSL_INSTALL_DIR = "${env:TARGET_DIR}\openssl-${env:OPENSSL_VERSION}-${address_model_target_dir_suffix}-vs2015-${env:OPENSSL_LINKAGE}"
      ...
      Write-Host "MSVC_CMD_BOOTSTRAP_OPTIONS  : ${env:MSVC_CMD_BOOTSTRAP_OPTIONS}"
