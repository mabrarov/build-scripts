...
foreach ($address_model in ${address_models}) {
  ...
  foreach ($openssl_linkage in ${openssl_linkages}) {
    ...
    $env:OPENSSL_BUILD_DIR = "${env:BUILD_DIR}\build\openssl-${env:OPENSSL_VERSION}\${address_model}\${env:OPENSSL_LINKAGE}"
    $env:OPENSSL_INSTALL_DIR = "${env:BUILD_DIR}\install\openssl-${env:OPENSSL_VERSION}\${address_model}\${env:OPENSSL_LINKAGE}"
