...
docker run --rm abrarov/win-builder:latest "%SEVEN_ZIP_HOME%/7z.exe"

Write-Host "Running MSYS2 GNU make in container created from abrarov/win-builder:latest image"
docker run --rm abrarov/win-builder:latest "%MSYS_HOME%\usr\bin\make" --version

Write-Host "Running ActivePerl in container created from abrarov/win-builder:latest image"
docker run --rm abrarov/win-builder:latest "%ACTIVE_PERL_HOME%\bin\perl.exe" --version

Write-Host "Running Python in container created from abrarov/win-builder:latest image"
docker run --rm abrarov/win-builder:latest "%PYTHON_HOME%\bin\python.exe" --version
